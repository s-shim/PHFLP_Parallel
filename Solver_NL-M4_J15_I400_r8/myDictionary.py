...
def EvaluatorNL2(theSelected,logSum,g,p,I,J,M,low,up):
    bestObj = 0.0
    pw = {}
    totalPW = {}
    for i in I:
    ...
    for (j,m) in theSelected:
        ...
        for i in I:
            ...
            choiceP[i,j,m] += (pw[i,j,m] / totalPW[i]) * pow(totalPW[i], logSum) / (1.0 + pow(totalPW[i], logSum))
        if LHS < 0:
            bestObj = bestObj + LHS
            feasibility = False
    ...
    return bestObj, feasibility, choiceP
